Blaster is a 3D multiplayer fast-paced shooter game developed on Unreal Engine 5, using Unreal Engine's Authoritative Client-Server model.
Developed while following Stephen Ulibarri’s Udemy course "UE5 C++ Multiplayer Shooter".

# How to play : 
- Download project via this repo.
- Package the project.
- Make sure Steam is opened on your PC, as it is used as the Online Subsystem to connect players in the game.
- Launch the Blaster.exe file from the packaged project folder.
- Host/Join a session with a friend !
- That's it ! enjoy :) 
