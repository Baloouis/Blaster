...
void ULagCompensationComponent::SaveFramePackage(FFramePackage& Package)
{
	...
	if (Character)
	{
		...
		for (auto& BoxPair : Character->HitCollisionBoxes)
		{
			...
			/*
			//Debug Draw
			FColor DrawColor = BoxPair.Key.Compare("head") == 0 ? FColor::Red : FColor::Blue;
			UBoxComponent* Box = Cast<UBoxComponent>(BoxPair.Value);
			if (Box)
			{
				DrawDebugBox(GetWorld(), Box->GetComponentLocation(), Box->GetScaledBoxExtent(), FQuat(Box->GetComponentRotation()), DrawColor, false, MaxRecordTime);
			}
			*/
		}
	}
...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
 {
 	...
 	if (World)
 	{
 		...
 		if (ConfirmHitResult.bBlockingHit) // we hit the head, return early ( for optimisation purposes )
 		{
 			
 			//Draw Debug Box On hit registered
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	TArray<FVector_NetQuantize> HeadshotHitLocations;
 		...
 		if (World)
 		{
 			...
 			if (BlasterCharacter)
 			{
 				...
 				*/
 				...
 				HeadshotHitLocations.Add(HitLocation);
 			}
 		}
 	}
 	...
 	for (auto& HitLocation : HitLocations)
 	{
 		// abort headshot hit locations to avoid double-damage from single hit
 		bool bShotAlreadyRegisteredAsHeadShot = false;
 		for (auto& HeadshotLocation : HeadshotHitLocations)
 		{
 			if (HeadshotLocation.Equals(HitLocation))
 			{
 				bShotAlreadyRegisteredAsHeadShot = true;
 				break;
 			}
 		}
 		if (bShotAlreadyRegisteredAsHeadShot)
 		{
 			//We should skip this Hit as it has been registered before as a headshot
 			continue;
 		}
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	//most recent registered Frame package was before HitTime
	//( most likely because the character hit is the server-controlled Character)
	//then it can be still be useful to check on the most recent frame.
