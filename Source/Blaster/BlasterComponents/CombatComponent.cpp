...
//Reload method is only called locally, on reload button is pressed
void UCombatComponent::Reload()
{
	if (CarriedAmmo > 0 && CombatState == ECombatState::ECS_Unoccupied && EquippedWeapon && !EquippedWeapon->IsFull()  && !bLocallyReloading)
	{
		ServerReload(); // warns server and other clients that player is reloading
		HandleReload(); // reload locally ( visually )
...
void UCombatComponent::ServerReload_Implementation()
{
	...
	//HandleReload();
	if (!Character->IsLocallyControlled()) HandleReload(); // Cause HandleReload has already been called before for locally controlled character
}
...
void UCombatComponent::OnRep_CombatState()
{
	...
	case ECombatState::ECS_SwappingWeapons:
		if (Character && !Character->IsLocallyControlled()) // if the character is Locally controlled then the swap montage has been called before in EquippedButtonPressed
		{
...
void UCombatComponent::ServerLaunchGrenade_Implementation(const FVector_NetQuantize& Target)
{
	if (Character && GrenadeClass && Character->GetAttachedGrenade())
	{
		...
		if (World)
		{
			...
			if (Grenade)
			{
				//Prevent Grenade from hitting owner character
...
void UCombatComponent::MulticastFire_Implementation(const FVector_NetQuantize& TraceHitTarget)
{
	...
	//this check makes sure we are either on the server or on a Client who is not controlling this character
	//as the character controlled locally on the Client would have already called LocalFire on its machine before
...
void UCombatComponent::LocalFire(const FVector_NetQuantize& TraceHitTarget)
{
	...
	//To Allow Firing while reloading Shotgun ( if CanFire() was true )
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	if (bScreenToWorld)
	{
		...
		if (Character)
		{
			...
			Start += CrosshairWorldDirection * ( DistanceToCharacter + TraceOffsetFromCharacter );
			if (bDrawShootingTraceStart)
			{
				DrawDebugSphere(GetWorld(), Start, 12.f, 12, FColor::Yellow);
			}
		}
...
//SwapWeapons is called from Server only, from ServerEquipButtonPressed_Implementation()
...
void UCombatComponent::InterpFOV(float DeltaTime)
{
	...
	if (bAiming)
	{
		CurrentFOV = FMath::FInterpTo(CurrentFOV, EquippedWeapon->GetZoomedFOV(), DeltaTime, EquippedWeapon->ZoomedInterpSpeed);
	}
	else
	{
...
void UCombatComponent::OnRep_Aiming()
{
	if (Character && Character->IsLocallyControlled())
	{
		// Client-Side Prediction for aiming bool
		// Discard the remote value to the local one, to avoid aiming glitch when there is lag
...
bool UCombatComponent::CanFire()
{
	...
	//To Allow Shooting Shotgun While Reloading it
...
void UCombatComponent::OnRep_CarriedAmmo()
{
	...
	//handle Shotgun reloading animation's end
